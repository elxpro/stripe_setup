defmodule StripeSetupWeb.PricingLive.IndexTest do
  ...
  describe "render/1" do
    test "renders log in page", %{conn: conn} do
      ...
      assert has_element?(lv, "h1", "$9.00")
      assert has_element?(lv, "h1", "$19.00")
      assert has_element?(lv, "h1>span", "per month")
      refute has_element?(lv, "h1>span", "per year")
  ...
  describe "handle_params/3" do
    test "renders log in page", %{conn: conn} do
      ...
      refute has_element?(lv, "h1>span", "per month")
      assert has_element?(lv, "h1>span", "per year")
  ...
  def setup_products_and_plans(_) do
    ...
    |> Enum.each(fn %{
      ...
      {:ok, product} =
        Billing.create_product(%{stripe_id: stripe_id, stripe_product_name: stripe_product_name})
